...
public extension SceneLocationView {
  ...
  func addPolylines(_ polylines: [MKPolyline]) {
    ...
    let polyNodes = polylines.map { PolylineNode(polyline: $0, altitude: altitude - 2.0) }
  ...
  func removePolylines(_ polylines: [MKPolyline]) {
    polylines.forEach { (polyline) in
      if let index = polylineNodes.index(where: { $0.polyline == polyline }) {
        polylineNodes.remove(at: index)
      }
    }
  }
